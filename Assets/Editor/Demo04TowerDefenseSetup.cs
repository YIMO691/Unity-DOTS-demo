...
namespace UnityDotsDemo.EditorTools
{
    ...
    public static class Demo04TowerDefenseSetup
    {
        ...
        public static void RepairDemo04()
        {
            ...
            bool force)
        {
            ...
            CreateOrUpdateGameState(subScene);
        ...
        private static List<Transform> CreateOrUpdatePath(Scene subScene, Material pathMaterial)
        {
            ...
            for (int i = 0; i < positions.Length; i++)
            {
                ...
                markerRenderer.enabled = false; // Debug-only, hidden by default.
            ...
            for (int i = 0; i < positions.Length - 1; i++)
            {
                ...
                segment.transform.localPosition = midpoint + Vector3.up * 0.15f;
            ...
            List<Transform> towerPoints)
        {
            ...
            authoring.Waves = CreateDefaultWaves();
        ...
        private static List<WaveDefinitionAuthoring> CreateDefaultWaves()
        {
            return new List<WaveDefinitionAuthoring>
            {
                CreateWave(5, 0, 0, 30f, 3f),
                CreateWave(8, 0, 0, 36f, 3f),
                CreateWave(5, 3, 0, 40f, 3f),
                CreateWave(10, 5, 0, 45f, 3.1f),
                CreateWave(8, 5, 2, 52f, 3.2f)
            };
        }

        private static WaveDefinitionAuthoring CreateWave(
            int normalCount,
            int fastCount,
            int bossCount,
            float normalHealth,
            float normalSpeed)
        {
            return new WaveDefinitionAuthoring
            {
        ...
        private static void CreateOrUpdateGameState(Scene subScene)
        {
            GameObject stateObject = FindRootObject(subScene, "DOTS Game State");
            if (stateObject == null)
            {
                stateObject = new GameObject("DOTS Game State");
        ...
        private static void CreateOrUpdateMainSceneObjects(Scene mainScene)
        {
            ...
            GameObject hudObject = FindRootObject(mainScene, "Demo04 Runtime UI");
            if (hudObject == null)
            {
                hudObject = new GameObject("Demo04 Runtime UI");
